Goldcoin Core version *0.15.1* is now available from:

  <https://download.goldcoin.org/goldcoin-0.15.1/>
...
A number of changes to the way Goldcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 silver per kB (0.00001 GLD). This is to prepare for dropping the minimum transaction fee to 0.1 silver per kB (0.0001 GLD) in the next release.
