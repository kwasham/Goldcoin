...
class TestNode():
    ...
    def _start_perf(self, profile_name=None):
        ...
        if not test_success('readelf -S {} | grep .debug_str'.format(shlex.quote(self.binary))):
            self.log.warning(
                "perf output won't be very useful without debug symbols compiled into faithcoind")
...
class TestNodeCLI():
    """Interface to goldcoin-cli for an individual node"""
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run goldcoin-cli command. Deserializes returned string as python object."""
        pos_args = [arg_to_cli(arg) for arg in args]
        named_args = [str(key) + "=" + arg_to_cli(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same goldcoin-cli call"
        ...
        self.log.debug("Running goldcoin-cli command: %s" % command)
